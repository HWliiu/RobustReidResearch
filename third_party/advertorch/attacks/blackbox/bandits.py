...
# !Modified (add data-dependent priors)
def bandit_attack(
    x,
    loss_fn,
    order,
    ...
    if prior_init is None:
        if downsampling is None:
            ...
            prior = x.new_zeros(prior_shape)
    else:
        prior = prior_init.clone()

    ndim = np.prod(list(prior.shape[1:]))

    for t in range(nb_iter):
        exp_noise = exploration * torch.randn_like(prior) / (ndim**0.5)

        # Query deltas for finite difference estimator
        q1 = F.interpolate(prior + exp_noise, size=adv.shape[-2:])
        q2 = F.interpolate(prior - exp_noise, size=adv.shape[-2:])
        q1 = F.normalize(torch.flatten(q1, 1), dim=-1).view_as(adv)
        q2 = F.normalize(torch.flatten(q2, 1), dim=-1).view_as(adv)
        ...
        grad_est = delta_L[:, None, None, None] * exp_noise
        if order == 2:
            # update prior
            prior = prior + online_lr * grad_est
            # make step with prior
            # note the (+): this indicates gradient ascent on the loss
            prior_tmp = F.interpolate(prior, size=x.shape[-2:])
            adv = adv + eps_iter * F.normalize(
                torch.flatten(prior_tmp, 1), dim=-1
            ).view_as(adv)
            # project
            delta = adv - x
            delta = projector(torch.flatten(delta, 1)[:, None, :]).view_as(delta)
        elif order == inf:
            ...
            # make step with prior
            prior_tmp = F.interpolate(prior, size=x.shape[-2:])
            adv = adv + eps_iter * torch.sign(prior_tmp)
            # project
            delta = adv - x
            delta = projector(torch.flatten(delta, 1)[:, None, :]).view_as(delta)
        else:
...
class BanditAttack(Attack, LabelMixin):
    ...
    def perturb(  # type: ignore
        self, x: torch.FloatTensor, y: Optional[torch.Tensor] = None
    ) -> torch.FloatTensor:
        """
        ...
        projector = _make_projector(eps, self.order, flat_x, clip_min, clip_max)
