...
class Misclassification(Criterion):
    ...
    def __init__(self, labels: Any):
        super().__init__()
        self.q_pids: ep.Tensor = ep.astensor(labels)

    def __repr__(self) -> str:
        return f"{self.__class__.__name__}({self.q_pids!r})"
    ...
    def __call__(self, perturbed: T, outputs: T) -> T:
        ...
        assert classes.shape == self.q_pids.shape
        is_adv = classes != self.q_pids
        return restore_type(is_adv)
