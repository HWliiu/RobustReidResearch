...
def adv_train(
    ...
    for batch_idx, (imgs, pids, camids) in enumerate(bar):
        ...
        for _ in range(adv_step):
            ...
            pairs = miner(adv_feats, pids)
            loss_t = criterion_t(adv_feats, pids, pairs)
...
def main():
    ...
    test_dataset_names = ["dukemtmcreid"]
    ...
    save_dir = Path(f"logs/fast_at/{model_name}/{'_'.join(train_dataset_names)}")
    save_dir.mkdir(parents=True, exist_ok=True)

    adv_steps = 4
    max_epoch = int(120 / adv_steps)
