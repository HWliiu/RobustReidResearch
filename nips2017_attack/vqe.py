...
class VQE:
    ...
    @torch.no_grad()
    def __call__(self, dataset, adv_dataset):
        sum_l2_dist = 0.0
        sum_lpips_value = 0.0
        sum_ssim_value = 0.0
        sum_psnr_value = 0.0
        ...
        adv_dataloader = DataLoader(adv_dataset, batch_size=64, num_workers=8)
        for (imgs, _), (adv_imgs, _) in zip(dataloader, adv_dataloader):
